...
// Create the real estate data as a separate module
// This avoids any potential issues with the data being recreated during component rendering
...
// Process the data once, outside of the component
const FORMATTED_TABLE_DATA = REAL_ESTATE_DATA.map((row) => {
  const price = row.closing_price;
  const formattedPrice = price
    ? price.replace(/\B(?=(\d{3})+(?!\d))/g, ',')
    : '';

  const roleActions =
    row.represented === 'Buyer'
      ? ['View Details', 'Contact Agent', 'Similar Properties']
      : ['View Details', 'Contact Agent', 'Schedule Tour'];

  return {
    name: row.address,
    email: `${row.city}, ${row.state} ${row.zip}`,
    avatar: {
      src: row.image_url,
      alt: `${row.address} property`,
    },
    createdDate: formattedPrice,
    addedDate: row.represented,
    fileSize: row.sold_date,
    link: row.zillow_link,
    actions: roleActions,
  };
});

// Define constants as module-level variables
...
const DEFAULT_TABLE_ROWS = FORMATTED_TABLE_DATA;

// Create the component using class syntax instead of hooks
// This completely avoids the useState hook issue
export class Table9 extends React.Component<
  React.ComponentPropsWithoutRef<'section'> & TableComponentProps
> {
  state = {
    currentPage: 1,
  };
